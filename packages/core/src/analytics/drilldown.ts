...
function teamDrilldownBase(
  ...
  if (!historicalPersonCohort(cohort)) {
    ...
    if (!teamResolved.includeCanceled) filters.push(sql`${schema.workflowState.category} <> 'canceled'`);
    return and(...filters) ?? sql`false`;
  }
  const person = personCohort(cohort.cohort);
  if (person === null) return teamDrilldownBase(principal, resolved, { ...cohort, cohort: 'current' });
  ...
  if (!historical.query.includeCanceled) filters.push(sql`${schema.workflowState.category} <> 'canceled'`);
...
export async function listAnalyticsDrilldown(
  ...
  if (!resolved.includeCanceled) rowBaseFilters.push(sql`${schema.workflowState.category} <> 'canceled'`);
