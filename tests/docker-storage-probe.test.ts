...
test('the storage probe uses the configured app origin and storage endpoint', async () => {
    ...
    async fetch(request) {
      ...
      if (request.method === 'OPTIONS') {
        ...
        return new Response(null, {
          status: 204,
          headers: offered === origin ? { 'access-control-allow-origin': origin } : {},
        });
      }
      if (request.method === 'PUT') {
        ...
        return new Response(null, { headers: { etag: '"probe"' } });
      }
