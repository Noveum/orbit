...
test.each(['https://orbit.example.test', 'https://old.example.test'])(
  'storage initialization preserves password characters and reconciles CORS from %s',
...
if (args.join(' ') === 'admin config get orbit api') process.stdout.write('api cors_allow_origin=${configuredOrigin} ');
`,
        { mode: 0o755 },
      );
      ...
      const updates = calls.filter(
        (call) => Array.isArray(call) && call[0] === 'admin' && call[2] !== 'get',
      );
      expect(updates).toEqual(
        configuredOrigin === 'https://orbit.example.test'
          ? []
          : [
              [
                'admin',
                ...
                'orbit',
                'api',
                'cors_allow_origin=https://orbit.example.test',
              ],
              ['admin', 'service', 'restart', 'orbit', '--json', '--wait'],
            ],
      );
    } finally {
